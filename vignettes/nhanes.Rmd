...
output: 
  rmarkdown::html_vignette:
    code_folding: hide
vignette: >
  %\VignetteIndexEntry{Race and Telomere Length}
...
knitr::opts_chunk$set(
  ...
  collapse = T, warning = F, error = F, message = F, comment = NA, cache = T)
...
FIG1_ACD <- TAB1_DAT_ACD |>
  ...
  ggplot(
    ...
    scale_fill_viridis_b(guide = F) +

    scale_color_viridis_b(guide = F) +

    scale_fill_jama(guide = F) +

    scale_color_jama(guide = F) +
...
TAB2a_ACD <- tbl_regression(fit_a_acd, TAB2_LABS_ACD, exponentiate = T) |>
...
TAB2b_ACD <- tbl_regression(wtd_fit_a_acd, TAB2_LABS_ACD, exponentiate = T) |>
...
des_svywtdlm <- svydesign(ids = ~ SDMVPSU, strata = ~ SDMVSTRA, nest = T,
                        
  weights = ~ WTMEC4YR, data = NHANES2)
...
des_pssvywtdlm <- svydesign(ids = ~ SDMVPSU, strata = ~ SDMVSTRA, nest = T,
                        
  weights = ~ comb_wt, data = NHANES2)
...
des_wtdpssvywtdlm <- svydesign(ids = ~ SDMVPSU, strata = ~ SDMVSTRA, nest = T,
                        
  weights = ~ wtd_comb_wt, data = NHANES2)
...
TAB2a_ATE <- tbl_regression(fit_a_ate, TAB2_LABS_ATE, exponentiate = T) |>
...
TAB2b_ATE <- tbl_regression(wtd_fit_a_ate, TAB2_LABS_ATE, exponentiate = T) |>
...
NHANES3$wtd_comb_wt <- with(NHANES3, WTMEC4YR*wtd_ps_wt)

#--- TABLE 2: PROPENSITY SCORE ANALYSIS RESULTS --------------------------------

TAB2_ATE <- tbl_merge(list(TAB2a_ATE, TAB2b_ATE),
...
des_wtdpssvywtdlm <- svydesign(ids = ~ SDMVPSU, strata = ~ SDMVSTRA, nest = T,
  ...
  weights = ~ wtd_comb_wt, data = NHANES3)
...
NHANES_RESULTS_ATE <- rbind(res_lm_ate, res_iptw_ate, res_svywtdlm_ate, 
                            
  res_iptwlm_ate, res_pssvywtdlm_ate, res_wtdpssvywtdlm_ate,
  
  fit_ID1_ate$cdiff[ c(1:2)],
  fit_ID2a_ate$cdiff[c(1:2)],
  fit_ID2b_ate$cdiff[c(1:2)]
)
